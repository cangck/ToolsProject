...
public class OkHttpUtils {
     ...
     * 加入调度 执行请求结果的回调
     */
    private void executeResult(final Request request, final ResultCallback callback){
         mOkHttpClient.build().newCall(request).enqueue(new Callback() {
             ...
             public void onResponse(Call call, Response response) throws IOException {
                     /**
                      * 只返回json类型的字符串，扩展性不行,不能写死只返回字符串
                      *
                      * 因为后台返回的不一定是string，可能是流对象等
                      * 所以请求的泛型是response，直接给你返回原始数据，再根据后台返回的数据类型
                      * 将response.body()方法 返回你需要的类型进行相应操作
                      */
//                     String str = response.body().string();
//                     if (callback.mType == String.class){
//                         sendSuccessCallBack(call,str);
//                     }else{}
                 ...
                 try {
                     ...
                     ToastUtil.showToast(MyApplication.getContext(), R.string.request_exception);
                 }
             }
         });
    }
     ...
     * @param
     */
    private void sendSuccessCallBack(final ResultCallback callback, final String response) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                if (callback != null) {
    ...
    private Request  buildPostRequest(String url, Map<String,String> map) {
      ...
      //  得到要构造的post body
//        RequestBody body = builder.build().create(JSON,params);
